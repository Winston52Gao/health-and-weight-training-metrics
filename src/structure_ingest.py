...
def structure_parser(pages: List[Dict[str, Any]], toc_map: Dict[int, str]) -> List[Dict[str, Any]]:
    ...
    # Unique TOC titles for fuzzy matching
    toc_titles = list(set(toc_map.values())) if toc_map else []

    # Persist section state across pages
    ...
    for p in pages:
        ...
        # Detect headings on page
        headings = detect_headings_on_page(raw)
        heading_texts = [h[0] for h in headings]

        page_toc_section = toc_map.get(page_no)

        # If we were in a transition and TOC now confirms the section,
        # restore full confidence.
        if (in_transition and active_section is not None and page_toc_section == active_section):
            current_section = active_section
            current_confidence = 0.9
            in_transition = False

        # Otherwise initialize from TOC when available.
        elif page_toc_section:
            current_section = page_toc_section
            current_confidence = 0.9
            active_section = current_section

        # Fall back to previously active section.
        else:
            current_section = active_section
            current_confidence = 0.25 if current_section is None else 0.6
        ...
        for para in parts:
            ...
            if first_line in heading_texts:
                ...
                # If TOC and heading agree on this page,
                # immediately restore confidence.
                if (
                    page_toc_section is not None
                    and matched_section == page_toc_section
                ):
                    current_confidence = 0.9
                    in_transition = False

                # Otherwise treat the remainder of the page
                # as a transition region.
                else:
                    in_transition = True

            para_entry["section_name"] = current_section
            para_entry["confidence"] = (
                current_confidence if current_section is not None else 0.25
            )
